...
export class FileStorageService {
    ...
    private async ensureStorageDirectory(): Promise<void> {
        ...
        try {
            ...
            this.logger.error(
                `Failed to create storage directories: ${errorMessage}`,
            );
            throw error;
        }
    }
    ...
    async readFileAsObject<T = Record<string, any>>(
        filePath: string,
    ): Promise<T> {
            ...
            switch (extension) {
                ...
                default:
                    throw new BadRequestException(
                        `Unsupported file format for parsing: ${extension}`,
                    );
            }
        } catch (error: unknown) {
    ...
    async writeFile(
        ...
        const processedBuffer = await this.processFileContent(
            buffer,
            filePath,
            options,
        );
            ...
            for (const item of items) {
                ...
                if (item.isDirectory() && recursive) {
                    ...
                    try {
                        ...
                        this.logger.warn(
                            `Failed to get metadata for ${relativePath}: ${error}`,
                        );
                    }
                }
            }
    ...
    async getFileMetadata(filePath: string): Promise<IFileMetadataModel> {
        ...
        try {
            ...
            this.handleFileError(
                error,
                `Failed to get file metadata: ${filePath}`,
            );
        }
    }
    ...
    async getThumbnail(
        filePath: string,
        width: number = this.defaultThumbnailSize,
        height?: number,
    ): Promise<Buffer> {
    ...
    async saveI18nFile(
        ...
        const normalizedFilename = filename.match(/\.(yaml|yml|json)$/)
            ? filename
            : `${filename}.yaml`;
    ...
    async getMediaByCategory(
        category: 'icons' | 'images' | 'videos' | 'audio',
    ): Promise<IFileMetadataModel[]> {
    ...
    private getFullPath(relativePath: string): string {
        ...
        if (normalized.startsWith('..') || path.isAbsolute(normalized)) {
            throw new BadRequestException(
                'Invalid file path: path traversal or absolute path not allowed',
            );
        }
    ...
    private convertContentToBuffer(
        content: string | Buffer | Record<string, any>,
        filePath: string,
    ): Buffer {
        if (Buffer.isBuffer(content)) {
    ...
    private detectFileCategory(extension: string): string {
        if (FileStorageService.SUPPORTED_IMAGE_FORMATS.includes(extension))
            return 'images';
        if (FileStorageService.SUPPORTED_VIDEO_FORMATS.includes(extension))
            return 'videos';
        if (FileStorageService.SUPPORTED_AUDIO_FORMATS.includes(extension))
            return 'audio';
        if (FileStorageService.SUPPORTED_I18N_FORMATS.includes(extension))
            return 'i18n';
    ...
    private async enrichMetadataWithSpecifics(
        fullPath: string,
        buffer: Buffer,
    ...
    private async addVideoMetadata(
        fullPath: string,
        metadata: IFileMetadataModel,
    ): Promise<void> {
        try {
    ...
    private async generateAndSaveThumbnail(
        ...
        try {
            const thumbnailBuffer = await this.generateImageThumbnail(
                buffer,
    ...
    private async generateVideoThumbnail(
        ...
        try {
            ...
            throw new BadRequestException(
                `Failed to generate video thumbnail: ${error}`,
            );
        }
    }
