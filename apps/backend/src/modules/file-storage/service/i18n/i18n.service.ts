...
export class I18nService {
    ...
    async getAvailableLanguages(): Promise<string[]> {
        try {
            ...
            const items = await fs.readdir(this.translationsPath, {
                withFileTypes: true,
            });
            ...
            for (const item of items) {
                if (item.isDirectory()) {
                    ...
                    try {
                        ...
                        this.logger.warn(
                            `No translations.json found for language: ${lang}`,
                        );
                    }
                }
            }
        ...
        try {
            ...
            try {
                ...
                throw new NotFoundException(
                    `translations.json not found for language: ${lang}`,
                );
            } catch {
    ...
    async getModuleTranslations(
        ...
        try {
            ...
            this.logger.error(
                `Error loading module translations for ${lang}/${module}:`,
                error,
            );
            return {};
        }
    ...
    async getTranslation(
        lang: string,
        key: string,
    ): Promise<{ value: string; module: string } | null> {
        try {
            const allTranslations = await this.getAllTranslations(lang, false);
            ...
            this.logger.error(
                `Error getting translation for key ${key}:`,
                error,
            );
            return null;
        }
    ...
    async getAvailableModules(lang: string): Promise<string[]> {
        try {
            ...
            this.logger.error(
                `Error getting modules for language ${lang}:`,
                error,
            );
            return [];
        }
