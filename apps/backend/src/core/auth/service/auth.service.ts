...
export class AuthService {
    ...
    async refreshToken(
        ...
        const session =
            await this.personSessionService.findActiveSessionByRefreshToken(
                refreshToken,
            );
    ...
    async getProfile(sessionId: number): Promise<LoginResponseClassDto> {
        const session =
            await this.personSessionService.findSessionById(sessionId);
    ...
    async validateUser(payload: JwtPayload): Promise<any> {
        ...
        const session = await this.personSessionService.findSessionById(
            payload.sessionId,
        );
    ...
    async validateSession(accessToken: string): Promise<boolean> {
        const session =
            await this.personSessionService.findActiveSessionByAccessToken(
                accessToken,
            );
        return !!session;
    }
