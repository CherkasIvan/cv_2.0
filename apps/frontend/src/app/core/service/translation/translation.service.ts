...
export class TranslationService {
  ...
  getTranslation(key: string, params?: Record<string, any>): string {
    const translation = this.getNestedTranslation(key, this.currentTranslations());
   ...
   */
  getModuleTranslations$(lang: string, module: string): Observable<Translation> {
    const url = this.buildUrl(`/${lang}/${module}`);
    
    return this.http.get<XliffFile>(url, { 
      responseType: 'json',
      headers: {
   ...
   */
  getAllModuleTranslations$(lang: Language): Observable<{ name: string; content: Translation }[]> {
   ...
   */
  getTranslationKey$(lang: string, key: string): Observable<{ value: string; module: string } | null> {
  ...
  private getNestedTranslation(key: string, obj?: Translation): string | Translation | undefined {
